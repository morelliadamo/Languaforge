...
--
-- Host: localhost:3306
-- Generation Time: Feb 23, 2026 at 12:38 PM
...
--
-- Database: `languaforge_db_23february`
--
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `create_lesson_progress` (IN `p_user_id` INT, IN `p_lesson_id` INT, IN `p_exercise_count` INT, IN `p_completed_exercises` INT)   BEGIN
  INSERT INTO lesson_progress (user_id, lesson_id, exercise_count, completed_exercises)
  VALUES (p_user_id, p_lesson_id, IFNULL(p_exercise_count, 0), IFNULL(p_completed_exercises, 0))
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `create_login` (IN `p_user_id` INT, IN `p_device_info` VARCHAR(255), IN `p_ip_address` VARCHAR(45), IN `p_session_token` VARCHAR(255), IN `p_expires_at` TIMESTAMP, IN `p_jti` VARCHAR(255))   BEGIN
  INSERT INTO login (user_id, device_info, ip_address, session_token, expires_at, jti)
  VALUES (p_user_id, p_device_info, p_ip_address, p_session_token, p_expires_at, p_jti);
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_friend` (IN `p_id` INT)   BEGIN
  SELECT * FROM friendship WHERE id = p_id AND is_deleted = 0;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `hard_delete_user` (IN `p_id` INT)   BEGIN
  ...
  -- FIX: was "DELETE FROM friend WHERE user_id = p_id OR friend_id = p_id"
  --      table is `friendship`, columns are user1_id / user2_id
  DELETE FROM friendship     WHERE user1_id = p_id OR user2_id = p_id;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `soft_delete_course` (IN `p_id` INT)   BEGIN
  IF EXISTS (
    SELECT 1 FROM unit WHERE course_id = p_id AND is_deleted = 0
  ) OR EXISTS (
    SELECT 1 FROM leaderboard WHERE course_id = p_id AND is_deleted = 0
  ) OR EXISTS (
    SELECT 1 FROM score
    WHERE is_deleted = 0
      AND lesson_id IN (
    ...
    SELECT 1 FROM review WHERE course_id = p_id AND is_deleted = 0
  )
  THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'Cannot delete course: related records exist.';
  ELSE
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `soft_delete_friend` (IN `p_id` INT)   BEGIN
  UPDATE friendship SET is_deleted = 1, deleted_at = NOW() WHERE id = p_id AND is_deleted = 0;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_friend_status` (IN `p_id` INT, IN `p_status` VARCHAR(50))   BEGIN
  UPDATE `friendship` SET status = p_status WHERE id = p_id AND is_deleted = 0;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_lesson_progress` (IN `p_id` INT, IN `p_completed_exercises` INT)   BEGIN
  UPDATE lesson_progress
  SET completed_exercises = p_completed_exercises,
      completed_at = CASE
        WHEN p_completed_exercises >= exercise_count AND exercise_count > 0
          THEN IFNULL(completed_at, NOW())
        ELSE completed_at
      END,
      updated_at = NOW()
  WHERE id = p_id AND is_deleted = 0;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_login` (IN `p_id` INT, IN `p_device_info` VARCHAR(255), IN `p_ip_address` VARCHAR(45), IN `p_session_token` VARCHAR(255), IN `p_expires_at` TIMESTAMP, IN `p_jti` VARCHAR(255))   BEGIN
  UPDATE login
...
INSERT INTO `login` (`id`, `user_id`, `login_time`, `device_info`, `ip_address`, `session_token`, `jti`, `expires_at`, `is_anonymized`, `anonymized_at`, `is_deleted`, `deleted_at`) VALUES
(1, 421, '2026-02-09 11:45:19', 'MAC: [-36, 70, 40, 50, -73, 17]; OS: Windows 11 10.0; Java: 25', NULL, NULL, NULL, '2026-02-09 12:45:19', 0, NULL, 0, NULL),
(2, 421, '2026-02-09 11:48:30', 'MAC: [-36, 70, 40, 50, -73, 17]; OS: Windows 11 10.0; Java: 25', NULL, NULL, NULL, '2026-02-09 12:48:30', 0, NULL, 0, NULL),
(3, 421, '2026-02-09 11:52:10', 'MAC: [-36, 70, 40, 50, -73, 17]; OS: Windows 11 10.0; Java: 25', NULL, NULL, NULL, '2026-02-09 12:52:10', 0, NULL, 0, NULL),
(4, 421, '2026-02-09 11:55:27', 'MAC: [-36, 70, 40, 50, -73, 17]; OS: Windows 11 10.0; Java: 25', NULL, NULL, NULL, '2026-02-09 12:55:27', 0, NULL, 0, NULL),
(5, 421, '2026-02-09 11:59:05', 'MAC: [-36, 70, 40, 50, -73, 17]; OS: Windows 11 10.0; Java: 25', NULL, NULL, NULL, '2026-02-09 12:59:05', 0, NULL, 0, NULL),
(6, 421, '2026-02-09 12:00:07', 'MAC: [-36, 70, 40, 50, -73, 17]; OS: Windows 11 10.0; Java: 25', NULL, NULL, NULL, '2026-02-09 13:00:07', 0, NULL, 0, NULL),
(7, 421, '2026-02-09 12:02:21', NULL, NULL, NULL, NULL, '2026-02-09 13:02:21', 1, NULL, 1, '2026-02-15 16:53:51'),
(8, 421, '2026-02-18 10:14:11', 'MAC: [-36, 70, 40, 50, -73, 17]; OS: Windows 11 10.0; Java: 25', '192.168.10.9', NULL, NULL, '2026-02-18 11:14:11', 0, NULL, 0, NULL),
(9, 427, '2026-02-21 16:27:49', 'MAC: [-36, 70, 40, 50, -73, 17]; OS: Windows 11 10.0; Java: 25', '192.168.0.13', NULL, NULL, '2026-02-21 17:27:49', 0, NULL, 0, NULL),
(10, 421, '2026-02-21 16:37:11', 'MAC: [-36, 70, 40, 50, -73, 17]; OS: Windows 11 10.0; Java: 25', '192.168.0.13', NULL, NULL, '2026-02-21 17:37:11', 0, NULL, 0, NULL);
...
(1, 'Free', '0.00', 'monthly', '2025-11-26 09:20:25', 1, '2036-03-01 15:14:34'),
...
INSERT INTO `store_item` (`id`, `name`, `description`, `type`, `price`, `is_deleted`, `deleted_at`) VALUES
(1, '5 Szív', 'Folytasd a tanulást!', 'hearts5', 1000, 0, NULL),
(2, '10 Szív', 'Érezd biztonságban magad!', 'hearts10', 1750, 0, NULL),
(3, '25 Szív', 'Garantált siker, végtelen lehetőség', 'hearts25', 3250, 0, NULL),
(4, '5 Tipp', 'Segít, ha elakadnál', 'hints5', 1000, 0, NULL),
(5, '10 Tipp', 'Nehezebb leckéknél hasznos a segítség', 'hints10', 1750, 0, NULL),
(6, '25 Tipp', 'Ne aggódj, ennyivel biztosan átugrod azt a leckét!', 'hints25', 3250, 0, NULL),
(7, 'Fagyasztás', 'Fagyaszd be a sikersorozatod, ha éppen nem érsz rá tanulni!', 'freeze', 1500, 0, NULL);

--
-- Triggers `store_item`
--
DELIMITER $$
CREATE TRIGGER `store_item_before_update_set_deleted_at` BEFORE UPDATE ON `store_item` FOR EACH ROW BEGIN
  IF OLD.is_deleted = 0 AND NEW.is_deleted = 1 THEN
    SET NEW.deleted_at = IF(NEW.deleted_at IS NULL, NOW(), NEW.deleted_at);
  END IF;
END
...
(1, 'maria_lee23', 'alex.jones@example.com', NULL, NULL, '$argon2id$v=19$m=5,t=2,p=2$rv63Vx8$C25hQlNAwSEuX02IA871IuuzK84C/R9i/8+4/9kgsmYyVA2FL4qpvFCnAxryzEr4WImolIAzuOVfcs+uk7/YIg', 1, '2025-08-11 07:44:32', NULL, 0, NULL, 0, NULL, NULL, 1),
(2, 'maria_lee', 'maria.lee@example.com', NULL, NULL, 'hash_pw2', 1, '2025-08-12 07:44:32', '2025-08-24 07:44:32', 0, NULL, 0, NULL, NULL, 1),
(4, 'Lakatos ALi2', 'lakatosALi2@gmail.hu', NULL, NULL, '$argon2id$v=19$m=5,t=2,p=2$LGYd2xo$Egi82HopA4DW9w9U0ubJwlO6ZunZAUgsKhVJ1XqP04n8eKHjgsY5d6VnxT4rXc273s+2tWaKPjOuCkhz4PKm3Q', 1, '2025-12-05 19:00:40', NULL, 0, NULL, 0, NULL, NULL, 1),
(5, 'JohnDoe', 'jdoe@example.com', NULL, NULL, '$argon2id$v=19$m=5,t=2,p=2$WKgqGbo$MpM9ndTyM1TrI0F2rXb0C5Fnq8UfBhG/B5kMwfrn4aJ2jWUUbpqnd/QlpUa8ffHolCXqMoejw9TI0SlIdw9dzQ', 1, '2025-12-08 10:13:02', NULL, 0, NULL, 0, NULL, NULL, 1),
(7, 'JohnDo3e', 'jdoe3@example.com', NULL, NULL, '$argon2id$v=19$m=5,t=2,p=2$reP0rKA$QS+hnhfn6skySU3pmS8U6uThzAdE3Yps6gMnBJh/ydo9ENcKEM/AVP3HGhfay45NPzCUzRsW9wXtIww2nVuo5g', 1, '2025-12-08 10:13:46', NULL, 0, NULL, 0, NULL, NULL, 1),
(8, 'JaneDeer', 'janedeer@example.com', NULL, NULL, '$argon2id$v=19$m=5,t=2,p=2$No7mPJI$0sUqn3wOyVstKeiR1y4ceY/aDSlZL89arvNQh+Si/ND6lDxog/8hZra6+hLGEJC4aynygMTuGeFVotCJ4lZmvQ', 1, '2025-12-08 10:16:32', NULL, 0, NULL, 0, NULL, NULL, 1),
(9, 'Lakatospeter24', 'lakatospeter@gmail.com', NULL, NULL, '$argon2id$v=19$m=5,t=2,p=2$YsPENLY$OfpQsEA5wS/TTLvUwpuk+WOeNB57kR9lsja7yaJemkyq8eJM61uApHjsX2Y8xXfG4Y+1b/p8i7LE2vVQe7CHsQ', 1, '2025-12-08 11:05:46', NULL, 0, NULL, 0, NULL, NULL, 1),
(10, 'lakatospeter25', 'lakatospeter25@gmail.com', NULL, NULL, '$argon2id$v=19$m=5,t=2,p=2$06oLROY$Bo6UeYQ8ImBDuZtwXIskDYOqGPHrJZRpWi5ZCHLo09Dis+vIDjwAk3MkrnNKRSai+h77Rh96C4b9EoxbPg9/rg', 1, '2025-12-08 11:06:25', NULL, 0, NULL, 0, NULL, NULL, 1),
(11, 'JohnnyDoe', 'johnnydoe@gmail.com', NULL, NULL, '$argon2id$v=19$m=5,t=2,p=2$6S9uWkA$DkPAedDoXpAwucAbYq8h4QdDIcgQTqqBmjdvchY3rs4TJz4Wu6JHN9dA+Ly3ysmEQti9O61it7Y0OIDojQ12Nw', 1, '2025-12-08 11:19:17', NULL, 0, NULL, 0, NULL, NULL, 1),
...
(423, 'KovacsLajos69', 'asdasd@gmail.com', NULL, NULL, NULL, 1, '2026-02-09 09:24:23', NULL, 0, NULL, 0, '2026-02-17 06:30:03', NULL, 0),
...
INSERT INTO `user_course` (`id`, `user_id`, `course_id`, `enrolled_at`, `progress`, `completed_at`, `is_deleted`, `deleted_at`) VALUES
(3, 424, 3, '2026-02-17 11:43:55', 0, NULL, 0, NULL),
(4, 425, 3, '2026-02-17 12:04:54', 0, NULL, 0, NULL),
(6, 427, 3, '2026-02-17 12:18:18', 0, NULL, 0, NULL),
(8, 429, 3, '2026-02-17 12:31:26', 0, NULL, 0, NULL),
(10, 421, 3, '2026-02-18 17:03:07', 0.4, NULL, 0, NULL);

--
-- Triggers `user_course`
--
DELIMITER $$
CREATE TRIGGER `user_course_before_update_set_deleted_at` BEFORE UPDATE ON `user_course` FOR EACH ROW BEGIN
  IF OLD.is_deleted = 0 AND NEW.is_deleted = 1 THEN
    SET NEW.deleted_at = IF(NEW.deleted_at IS NULL, NOW(), NEW.deleted_at);
  END IF;
END
...
ALTER TABLE `login`
  ...
  ADD UNIQUE KEY `idx_login_jti` (`jti`),
...
ALTER TABLE `user_item`
  ADD PRIMARY KEY (`id`),
  ADD KEY `fk_user_item_user` (`user_id`),
  ADD KEY `fk_user_item_item` (`item_id`);
...
--
ALTER TABLE `user_item`
  ADD CONSTRAINT `fk_user_item_item` FOREIGN KEY (`item_id`) REFERENCES `store_item` (`id`),
  ADD CONSTRAINT `fk_user_item_user` FOREIGN KEY (`user_id`) REFERENCES `user` (`id`);
